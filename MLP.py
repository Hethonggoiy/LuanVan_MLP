...
# - Bổ sung: Lưu cấu hình MLP tốt nhất vào file mlp_config.txt khi đạt tối ưu
# =====================================================================
...
def train_mlp(X, y_encoded, le, progress_callback=None):
    ...
    # Bổ sung: Lưu cấu hình MLP vào file .txt khi đạt tối ưu
    config_content = f"MLP Configuration (Accuracy: {acc*100:.2f}%)\n"
    config_content += f"hidden_layer_sizes = {mlp.hidden_layer_sizes}\n"
    config_content += f"activation = {mlp.activation}\n"
    config_content += f"solver = {mlp.solver}\n"
    config_content += f"alpha = {mlp.alpha}\n"
    config_content += f"batch_size = {mlp.batch_size}\n"
    config_content += f"learning_rate = {mlp.learning_rate}\n"
    config_content += f"learning_rate_init = {mlp.learning_rate_init}\n"
    config_content += f"max_iter = {mlp.max_iter}\n"
    config_content += f"early_stopping = {mlp.early_stopping}\n"
    config_content += f"validation_fraction = {mlp.validation_fraction}\n"
    config_content += f"n_iter_no_change = {mlp.n_iter_no_change}\n"
    config_content += f"random_state = {mlp.random_state}\n"

    with open(CONFIG_FILE, 'w') as f:
        f.write(config_content)
    logging.info(f"Đã lưu cấu hình MLP vào {CONFIG_FILE}")

    if progress_callback:
        progress_callback(100)

    return mlp, acc, report, cm, le.classes_
...
class App:
    ...
    def create_widgets(self):
        ...
        labels = ["Họ và tên:", "Giới tính:", "Năm sinh:"]
        self.vars_info = [tk.StringVar() for _ in labels]
    ...
    def train(self):
        ...
        self.mlp_model, self.acc, self.report, self.cm, self.classes = train_mlp(X, y_encoded, le, progress_update)

        msg = f"Hoàn tất huấn luyện!\nAccuracy trên tập test: {self.acc*100:.2f}%\nCấu hình MLP đã lưu vào {CONFIG_FILE}"
        messagebox.showinfo("Kết quả huấn luyện", msg)
        self.status_var.set(f"MLP đạt {self.acc*100:.2f}% - Sẵn sàng gợi ý")
    ...
    def show_report(self):
        ...
        content += "=" * 80 + "\n\n"
        content += f"Accuracy trên tập test: {self.acc*100:.2f}%\n\n"
